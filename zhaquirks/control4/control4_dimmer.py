    ...
    async def bind(self):
        try:
            await _c4_report_controller_identity(
                ...
                "identify_pre_bind_DimmerOnOff",
                zcl_seq=self.endpoint.device.get_sequence(),
            )
            _LOGGER.info("C4 DimmerOnOff: identity sent")
        except Exception as e:
            _LOGGER.warning(
                "C4 DimmerOnOff: identity send failed (%s), continuing", e
            )
        ...
        try:
            ...
            _LOGGER.info("C4 DimmerOnOff: many-to-one route request sent")
        except Exception as e:
            _LOGGER.warning(
                "C4 DimmerOnOff: route request failed (%s), continuing", e
            )
